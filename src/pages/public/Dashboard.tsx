  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      <div className="max-w-[1280px] mx-auto p-4 md:p-6 flex flex-col gap-6 w-full pb-20">
        ...
        <section
          className="relative w-full h-[300px] md:h-[400px] rounded-[2.5rem] overflow-hidden shadow-sm flex flex-col justify-end p-6 md:p-12 group"
          style={{
            backgroundImage:
        ...
        >
          ...
          <div className="relative z-10 text-white max-w-2xl">
            <span className="inline-block px-3 py-1 bg-primary/80 backdrop-blur-md rounded-full font-label text-[10px] font-black uppercase tracking-widest mb-4 text-on-primary">
              Featured Destination
            </span>
            <h2 className="font-display text-4xl md:text-6xl font-black text-white mb-4 leading-tight italic uppercase">
              Santorini, Greece
            </h2>
            <p className="font-body text-base text-white/80 mb-8 max-w-md line-clamp-2 md:line-clamp-none italic">
              Experience the breathtaking sunsets and iconic white architecture
        ...
        {(loading || trips.length > 0) && (
          <section className="space-y-6 mt-8">
            <div className="flex items-center justify-between">
              <h2 className="font-headline text-2xl font-black text-on-surface italic uppercase tracking-tighter">
                My Active Trips
              </h2>
              <button className="text-primary font-label text-xs font-black uppercase tracking-widest hover:underline cursor-pointer">
                View All
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {loading
                ? [1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className="h-28 bg-surface-canvas animate-pulse rounded-2xl border border-border-subtle"
                    ></div>
                  ))
                : trips.map((trip) => (
                    <div
                      key={trip.id}
                      className="bg-surface-canvas p-5 rounded-2xl border border-border-subtle shadow-sm flex items-center gap-5 hover:border-primary transition-all cursor-pointer group hover:shadow-md"
                    >
                      ...
                      <div className="min-w-0">
                        <h3 className="font-headline text-base font-bold text-text-primary truncate uppercase">
                          {trip.title}
                        </h3>
                        <div className="flex items-center gap-1.5 mt-1">
                          <MapPin size={12} className="text-text-secondary" />
                          <p className="font-body text-[10px] text-text-secondary truncate font-bold uppercase tracking-wider">
                            {trip.destination}
                          </p>
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                          <span
                            className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-[0.1em] shadow-sm ${
                              trip.status === "ongoing"
        ...
        <section className="space-y-8 mt-10">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <h2 className="font-headline text-3xl font-black text-on-surface italic uppercase tracking-tighter">
          ...
          {processedDestinations.length > 0 ? (
              ...
              {processedDestinations.map((dest) => (
                ...
                >
                  ...
                  <div className="p-8 flex flex-col flex-1 gap-4">
                    <div className="flex items-center justify-between">
                      <h3 className="font-headline text-2xl font-black text-text-primary uppercase truncate pr-4">
                        {dest.title}
                      </h3>
                      <div className="flex items-center gap-1.5 bg-yellow-50 px-3 py-1 rounded-full border border-yellow-200">
                        ...
                        />
                        <span className="font-display text-xs font-black text-on-surface">
                          {dest.rating}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto flex items-center justify-between pt-6 border-t border-border-subtle/50 gap-4">
                      <div className="min-w-0">
                        <span className="text-[10px] text-text-secondary font-black uppercase tracking-[0.2em] block mb-1">
                          Starting from
                        </span>
                        <p className="text-2xl font-display font-black text-primary truncate">
                          {dest.price}
                        </p>
                      </div>
                      <div className="flex gap-2 shrink-0">
                        <a
                          href={
                            dest.mapUrl ||
                            `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(dest.title)}`
                          }
                          target="_blank"
            ...
            <div className="py-32 text-center bg-surface-canvas rounded-[4rem] border-2 border-dashed border-border-subtle flex flex-col items-center">
              ...
              <h3 className="font-headline text-2xl font-black text-on-surface uppercase opacity-30 italic">
                No destinations matched your search
              </h3>
              <p className="font-body text-sm text-text-secondary mt-2 opacity-50">
                Try using different keywords or exploring another category.
              </p>
              <button
                onClick={() => {
                  setSearchQuery("");
