...
export function Layout() {
  ...
  React.useEffect(() => {
    const handleOpenModal = (e: any) => {
      setPrefilledPlace(e.detail?.place || "");
    ...
    <div className="bg-surface-background text-on-surface font-body min-h-screen flex w-full">
      ...
      <nav className="hidden md:flex flex-col h-screen w-64 fixed left-0 top-0 border-r border-border-subtle bg-surface-container-low py-6 px-4 z-50">
        ...
        <div className="flex flex-col gap-1 flex-grow overflow-y-auto overflow-x-hidden pr-2 -mr-2">
          ...
          <Link to="/" className={getLinkClasses(["/", "/builder"])}>
          ...
          <Link to="/itinerary#ongoing" className={getLinkClasses(["/itinerary"], "#ongoing")}>
            <PlayCircle className="w-5 h-5 text-itinerary-stay" />
            <span className="font-body text-sm">Ongoing (1)</span>
          </Link>
          <Link to="/itinerary#upcoming" className={getLinkClasses(["/itinerary"], "#upcoming")}>
            <Clock className="w-5 h-5 text-primary" />
            <span className="font-body text-sm">Upcoming (2)</span>
          </Link>
          <Link to="/itinerary#completed" className={getLinkClasses(["/itinerary"], "#completed")}>
          ...
          <div className="flex flex-col mt-6 bg-surface-container-highest/20 rounded-xl p-2 pb-3 mb-2">
            ...
            <div className="flex flex-col gap-1">
              <Link to="/itinerary" className={getLinkClasses(["/itinerary"], "")}>
                <Calendar className="w-5 h-5" />
                <span className="font-body text-sm">Itinerary</span>
              </Link>
              <Link to="/budget" className={getLinkClasses(["/budget"])}>
                <Wallet className="w-5 h-5" />
                <span className="font-body text-sm">Budget</span>
              </Link>
              <Link to="/notes" className={getLinkClasses(["/notes"])}>
                <FileText className="w-5 h-5" />
                <span className="font-body text-sm">Trip Notes</span>
              </Link>
              <Link to="/profile" className={getLinkClasses(["/profile", "/settings"])}>
      ...
      <nav className="md:hidden bg-surface-container-lowest shadow-lg rounded-t-xl fixed bottom-0 w-full z-50 flex justify-around items-center px-4 py-2 border-t border-border-subtle">
        ...
        <Link to="/itinerary" className={getMobileLinkClasses(["/itinerary", "/builder"])}>
          <PlaneTakeoff className="w-6 h-6 mb-1" />
          <span className="font-label text-[10px]">Trips</span>
        </Link>
        <Link to="/community" className={getMobileLinkClasses(["/community"])}>
